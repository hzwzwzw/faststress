...
class TestToArgs:
    ...
    def test_backend_oai(self):
        case = TestCase(name="oai")
        server = ServerConfig(backend=Backend.SGLANG_OAI)
        args = case.to_bench_args(server)
        assert "sglang-oai" in args

    def test_api_key_env(self):
        server = ServerConfig(api_key="sk-test123")
        env = TestCase.get_env(server)
        assert env == {"OPENAI_API_KEY": "sk-test123"}

    def test_no_api_key_env(self):
        server = ServerConfig()
        assert TestCase.get_env(server) is None
...
class TestBackwardCompat:
    def test_old_json_with_server_field(self):
        """Old JSON with server field inside TestCase should be loadable."""
        case = TestCase.model_validate({
            "name": "old-case",
            "server": {"host": "10.0.0.1", "port": 8000},
            "dataset": {"dataset_type": "random-ids"},
            "load": {"num_prompts": 50},
        })
        assert case.name == "old-case"
        assert case.load.num_prompts == 50
